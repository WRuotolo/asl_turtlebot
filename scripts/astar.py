...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        for i in range(-1,2):
            for j in range(-1,2):
                if self.is_free((x[0] + i * self.resolution, x[1] + j * self.resolution)) and not (i == 0 and j == 0):
                    valid_neighbors.append(self.snap_to_grid((x[0]+i*self.resolution, x[1]+j*self.resolution)))
